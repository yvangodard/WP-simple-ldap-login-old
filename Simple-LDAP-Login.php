...
Plugin URI: https://github.com/yvangodard/Wordpress-simple-LDAP-Login
Description:  Authenticate WordPress against LDAP, original plugin (http://clifgriffin.com/simple-ldap-login/) modded to support attribute "resMemberOf" for groups of users by J. Lafforgue (http://www.imjweb.fr)
Version: 1.5.5
Author: Clif Griffin Development Inc. - Modded by Jerôme LAFFROGUE 
...
class SimpleLDAPLogin {
	...
	function user_has_groups( $username = false, $directory ) {
		...
		if ( $directory == "ad" ) {
			...
			$result = ldap_search($this->ldap, $this->get_setting('base_dn'), '(' . $this->get_setting('ol_login') . '=' . $username . ')', array('resMemberOf'));
			...
			$user_groups = array();
			for ( $i = 0; $i < $ldapgroups[0]['resmemberof']['count']; $i++) {
				$user_groups[] = $ldapgroups[0]['resmemberof'][$i];
			}
